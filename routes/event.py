...
from database.event import decline_invite, accept_invite, retrieve_mutual_events, retrieve_user_invitees_event, \
    retrieve_user_event, retrieve_events, update_event_details, add_event_to_users, remove_user_event,\
    EventNotFoundException, retrieve_user_events
from database.user import UserNotFoundException
from models.event import EventModel, UpdateEventModel, ResponseModel, ErrorResponseModel
...
def post_user_event(user_id: str, event: EventModel = Body(...)):
    try:
        ...
        new_event = add_event_to_users(user_id, event)
        return ResponseModel(new_event, "User event added successfully.")
